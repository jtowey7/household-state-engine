...
function maps(rows: Row[]): Map<string, { item: string; recipeUnit: string; canonicalUnit: string }> {
  ...
  for (const row of rows) {
    ...
    const recipeUnit = value(f, "fldSTLFCWqngzNdim", "Recipe unit");
    const canonicalUnit = value(f, "fldPOD5tElUhQf23L", "Canonical unit");
    const active = value(f, "fldUCqeoFEOMbcjaA", "Active");
    if (active === true && typeof alias === "string" && typeof item === "string" && typeof recipeUnit === "string" && typeof canonicalUnit === "string") {
      if (result.has(alias) && result.get(alias)!.item !== item) throw new Error(`Delivery inventory materialisation refused: ambiguous ITEM KEY MAP alias ${alias}.`);
      result.set(alias, { item, recipeUnit, canonicalUnit });
    }
  }
  return result;
}
...
export function buildDeliveryProjection(eventRows: AirtableRow[], mapRows: AirtableRow[]): InventoryProjection[] {
  ...
  const lookup = maps(mapRows);
  const byItem = new Map<string, InventoryProjection>();
  for (const event of mapped.events) {
    ...
    if (prior) {
      if (prior.unit !== targetUnit) throw new Error(`Delivery inventory materialisation refused: unit conflict for ${targetItem}.`);
      ...
      byItem.set(targetItem, { item: targetItem, quantity, unit: targetUnit, sourceEventIds: [event.eventId], occurredAt: event.occurredAt });
    }
  }
...
function existingInventory(rows: Row[]): Map<string, Row> {
  ...
  for (const row of rows) {
    const item = value(row.fields, "fld58iyqxlpG04WGN", "Item");
    if (typeof item !== "string" || !item.trim()) continue;
    if (result.has(item)) throw new Error(`Delivery inventory materialisation refused: duplicate Production INVENTORY item identity ${item}.`);
    result.set(item, row);
  }
  return result;
}
...
export function buildInventoryUpdates(projection: InventoryProjection[], inventoryRows: AirtableRow[], deliveredDate: string): Array<{ id?: string; fields: Record<string, unknown>; item: string; applied: boolean }> {
  const existing = existingInventory(inventoryRows);
  const updates: Array<{ id?: string; fields: Record<string, unknown>; item: string; applied: boolean }> = [];
...
export async function executeProductionDeliveryInventory(env: Record<string, string | undefined>, fetchImpl: FetchLike = fetch as FetchLike): Promise<{ ok: true; applied: number; skipped: number; projectionFingerprint: string }> {
  ...
  const events = await listRows(fetchImpl, apiKey, baseId, EVENTS, EVENT_FIELDS);
  const inventory = await listRows(fetchImpl, apiKey, baseId, INVENTORY, INVENTORY_FIELDS);
  const itemMaps = await listRows(fetchImpl, apiKey, baseId, ITEM_KEY_MAP, MAP_FIELDS);
  const deliveryEvents = eventRowsForDelivery(events);
  if (deliveryEvents.length !== 22) throw new Error(`Production delivery inventory materialisation refused: expected exactly 22 pending approved delivery events, found ${deliveryEvents.length}.`);
  const projection = buildDeliveryProjection(deliveryEvents, itemMaps);
  const fingerprint = hashOf(projection);
  const updates = buildInventoryUpdates(projection, inventory, required(env, "FOODOS_DELIVERY_DATE"));
  const toWrite = updates.filter((u) => u.applied).map(({ id, fields }) => ({ id, fields }));
  await writeBatch(fetchImpl, apiKey, baseId, toWrite);
