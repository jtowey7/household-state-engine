...
export async function createOperatorSession(token: string, env: Environment): Promise<Response> {
  ...
  if (!expected) {
    return Response.json({ ok: false, code: "SERVER_NOT_CONFIGURED", missingConfiguration: missingServerConfiguration(env), error: notConfiguredMessage(env) }, { status: 503 });
  }
  if (!(await constantTimeTokenMatch(normaliseAccessCode(token), expected))) {
    return Response.json({ ok: false, code: "INVALID_CREDENTIAL", error: "That access code does not match the one configured for this deployment.", detail: describeAccessCodeMismatch(token, expected) }, { status: 401 });
  }
  ...
  return new Response(JSON.stringify({ ok: true, expiresAt: new Date((issuedAt + MAX_AGE_SECONDS) * 1000).toISOString(), missingConfiguration: missingServerConfiguration(env) }), {
    status: 200,
    headers: { "content-type": "application/json", "set-cookie": `${COOKIE_NAME}=${session}; Max-Age=${MAX_AGE_SECONDS}; Path=/; HttpOnly; Secure; SameSite=Strict`, "cache-control": "no-store" },
  });
}
...
export async function authorizeOperatorSession(request: Request, env: Environment): Promise<Response | undefined> {
  ...
  if (!Number.isInteger(issuedAt) || !nonce || !signature) return operatorAuthorizationFailure("Invalid operator session", 401);
  const age = Math.floor(Date.now() / 1000) - issuedAt;
  if (age < 0 || age > MAX_AGE_SECONDS) return operatorAuthorizationFailure("Operator session expired", 401);
  const payload = `${issuedAt}.${nonce}`;
  ...
  if (!providedBytes || !expectedBytes || providedBytes.length !== expectedBytes.length) return operatorAuthorizationFailure("Invalid operator session", 401);
  let diff = 0;
...
export function clearOperatorSession(): Response {
  return new Response(JSON.stringify({ ok: true }), {
    status: 200,
    headers: { "content-type": "application/json", "set-cookie": `${COOKIE_NAME}=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Strict`, "cache-control": "no-store" },
  });
}
