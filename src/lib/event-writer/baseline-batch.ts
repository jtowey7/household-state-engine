...
export async function appendBaselineBatch(
  ...
  const reject = (code: "BATCH_AUTHORIZATION_INVALID" | "BATCH_AUTHORIZATION_SCOPE_MISMATCH" | "TEST_RECORD_REFUSED", detail: string): AppendReceipt[] => [
    {
  ...
  if (authorization.decision !== "APPROVED" || !["EXPLICIT_USER_INPUT", "STRONG_TRANSACTION_EVIDENCE"].includes(authorization.evidenceSource)) {
