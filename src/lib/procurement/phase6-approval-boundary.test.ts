...
function plan(): QuantityRunPlan {
  return {
    ...
    rejections: [], blockedItemKeys: [],
  };
}
function basket() { return aggregateCandidateBasket(plan(), { catalogue: shadowCatalogue, retailer: "synthetic-grocer" }); }
function evidence(candidate: ReturnType<typeof basket>) { return dispatchEvidence(candidate.totalCost, "2026-08-22T00:24:00.000Z"); }
...
describe("Basket Phase 6 approval boundary acceptance", () => {
  it("keeps dispatch intent creation fail-closed until a human approval exists", () => {
    const candidate = basket(); const approval = createBasketApproval(candidate);
    expect(() => createDispatchIntent(approval, candidate, INTENT_AT, evidence(candidate))).toThrow("Cannot create dispatch intent: NOT_APPROVED");
  });
  it("creates a bounded dispatch intent only from a valid, versioned human approval", () => {
    const candidate = basket(); const approved = approveBasket(createBasketApproval(candidate), candidate, "James", APPROVED_AT, NOW);
    expect(validateBasketApproval(approved, candidate, NOW)).toEqual({ valid: true });
    const intent = createDispatchIntent(approved, candidate, INTENT_AT, evidence(candidate));
    expect(intent.status).toBe("READY"); expect(intent.requiresExternalDispatch).toBe(true); expect(intent.basketId).toBe(candidate.basketId);
    expect(intent.basketVersion).toBe(approved.basketVersion); expect(intent.basketFingerprint).toBe(approved.basketFingerprint);
  ...
  it("invalidates approval when the basket materially changes before dispatch", () => {
    ...
    expect(() => createDispatchIntent(approved, changed, INTENT_AT, evidence(changed))).toThrow("Cannot create dispatch intent: BASKET_CHANGED");
  });
  it("rejects approval provenance without silently widening the authority boundary", () => {
    const candidate = basket(); const pending = createBasketApproval(candidate);
    expect(() => approveBasket(pending, candidate, "   ", APPROVED_AT, NOW)).toThrow("Cannot approve basket: APPROVAL_ACTOR_REQUIRED");
    expect(() => approveBasket(pending, candidate, "James", "2026-08-22T00:31:00.000Z", NOW)).toThrow("Cannot approve basket: APPROVAL_TIMESTAMP_FUTURE");
  });
});
