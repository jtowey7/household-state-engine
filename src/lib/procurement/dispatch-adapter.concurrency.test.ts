...
describe("TEST dispatch adapter: concurrent idempotency", () => { it("returns one identical receipt for concurrent reuse of the same dispatch intent", async () => { const { basket, approval, evidence } = approved(); const intent = createDispatchIntent(approval, basket, "2026-08-17T12:01:00.000Z", evidence); const adapter = createTestDispatchAdapter({ acceptedAt: "2026-08-17T12:02:00.000Z" }); const receipts = await Promise.all(Array.from({ length: 8 }, () => adapter.dispatch(intent, approval, basket))); expect(new Set(receipts.map((r) => r.dispatchId)).size).toBe(1); expect(new Set(receipts.map((r) => r.externalOrderId)).size).toBe(1); expect(new Set(receipts.map((r) => r.acceptedAt)).size).toBe(1); expect(receipts.every((r) => r.status === "ACCEPTED")).toBe(true); }); });
