...
const plan: QuantityRunPlan = {
  replayId: "PHASE7-ACCEPTANCE-R1",
  snapshotId: "PHASE7-ACCEPTANCE-S1",
  replayTimestamp: "2026-08-17T22:00:00.000Z",
  reconciliationStatus: "CLEAN",
  planId: "PHASE7-ACCEPTANCE-P1",
  ...
  requirements: [
    {
      ...
      onHandQuantity: 800,
      targetQuantity: 2000,
      sourceEventIds: ["PHASE7-ACCEPTANCE-EVT-1"],
      packSize: 500,
      packCount: 3,
...
describe("Phase 7 Order/dispatch acceptance", () => {
  it("proves the complete TEST dispatch lifecycle remains non-retailer and idempotent", async () => {
    const { basket, approval } = buildApprovedBasket();
    ...
    const repeat = await adapter.dispatch(intent, approval, basket);
