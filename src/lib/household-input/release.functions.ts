...
export function createAirtableGatewayFetch(options: {
  ...
  return async (input, init) => {
    ...
    return fetchImpl(url, {
      ...
      headers: {
        ...
        "X-Connection-Api-Key": options.connectionApiKey,
...
export function scopeMaterialisationPlan(plan: MaterialisationPlan, itemKey: string): MaterialisationPlan {
  ...
  return {
    ...
    lines,
    writes,
    eventIdsToMarkReplayed: plan.eventIdsToMarkReplayed.filter((eventId) => eventIds.has(eventId)),
  ...
  .handler(async ({ data }): Promise<HouseholdIntakeReleaseResult & { materialisation?: unknown }> => {
    ...
    if (!baseId || !credential || !lovableApiKey) {
      ...
      return {
        ...
        detail: "Production Airtable connector is not configured; no household state was written.",
      };
    }
    ...
    if (result.ok && data.submission.kind === "STOCK_CORRECTION" && (result.appended > 0 || result.duplicates > 0)) {
      ...
      const scope = {
        ...
        windowStart: MATERIALISATION_WINDOW_START,
        windowEnd: data.preparedAt,
      };
      const eventsTable = env["AIRTABLE_HOUSEHOLD_EVENTS_TABLE"] || HOUSEHOLD_EVENTS_TABLE;
      const readPort = createAirtableRestRowSource({
        config: {
          apiKey: credential,
          baseId,
          eventsTable,
          apiUrl: AIRTABLE_GATEWAY_URL,
        },
        fetchImpl: gatewayFetch,
      });
      ...
      try {
        ...
        const approval: MaterialisationApproval = {
          approvalId: `MAT-${data.approvals[0]?.authorizationId ?? report.exceptionId}`,
          approvedBy: approvedBy ?? "household operator",
          approvedAt: data.approvals[0]?.approvedAt ?? data.preparedAt,
          expectedSnapshotId: snapshot.snapshotId,
          expectedReplayId: snapshot.replayId,
        };
        const fullPlan = planInventoryMaterialisation({
          loaded,
          snapshot,
          existingInventory: await writePort.listInventory(),
          approval,
        });
        if (!fullPlan.ok) {
        ...
        const materialisation = execution.ok
          ? { ok: true, stage: "EXECUTE", materialisationId: execution.materialisationId, created: execution.created, updated: execution.updated, unchanged: execution.unchanged, replayStatusUpdatedEventIds: execution.replayStatusUpdatedEventIds }
          : { ok: false, stage: "EXECUTE", code: execution.code, detail: execution.detail, created: execution.created, updated: execution.updated };
        setResponseStatus(execution.ok ? 200 : 502);
        setResponseHeader("Cache-Control", "no-store");
        return { ...result, materialisation };
      } catch (cause) {
