...
function CyclePage() {
  ...
  return <div className="ctl-page"><AppHeader eyebrow="Your food" /><Shell>
    ...
    {meals.length === 0 ? <section className="mb-7 rounded-2xl bg-[var(--ctl-surface-sunken)] p-5"><SectionHeading title="Nothing planned yet" /><p className="text-[13px] leading-relaxed text-muted-foreground">There are no meals for this week. Answer two quick questions and foodOS will suggest a week for you to pick from.</p><Link to="/plan-week" className="mt-4 inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Plan this week</Link></section> : null}
    ...
    <section className="mb-7"><SectionHeading title="Shopping" action={<Pill tone={cycle.tone}>{cycle.shoppingStatus}</Pill>} /><Group><Row><div className="flex items-center justify-between gap-3"><div><p className="text-[15px] font-semibold">{basket?.status === "READY" ? basket.basket.retailer : "This week's shop"}</p><p className="mt-1 text-[13px] text-muted-foreground">{basketReady ? `${basket.basket.lines.length} things to buy · ${cycle.shopping}` : cycle.shopping}</p></div>{cycle.shopViewable ? <Link to="/shop" className="text-[13px] font-medium text-primary underline-offset-4 hover:underline">View</Link> : null}</div></Row></Group></section>

    <section className="mb-7"><SectionHeading title="Delivery" action={<Pill tone={cycle.tone}>{cycle.stage === "ALL_SETTLED" ? "Counted in" : "Not counted in yet"}</Pill>} /><Group><Row><div className="flex items-center justify-between gap-3"><div><p className="text-[15px] font-semibold">{delivery?.status === "READY" ? delivery.basket.retailer : "Your delivery"}</p><p className="mt-1 text-[13px] text-muted-foreground">{cycle.delivery}</p></div>{receiptConfirmed || !deliveryReady ? null : <Link to="/delivery" className="text-[13px] font-medium text-primary underline-offset-4 hover:underline">Check</Link>}</div></Row></Group></section>
