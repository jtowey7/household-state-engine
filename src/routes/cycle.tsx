...
function CyclePage() {
  ...
  return <div className="ctl-page"><AppHeader eyebrow="Your food" /><Shell>
    ...
    <section className="mb-7 rounded-2xl border border-primary/20 bg-[var(--ctl-surface-sunken)] p-5"><div className="flex items-start justify-between gap-4"><div><SectionHeading title="Next up" /><p className="mt-2 text-[15px] font-semibold">{nextAction.label}</p><p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">FoodOS keeps the next useful household step in front of you.</p></div><Link to={nextAction.to} className="shrink-0 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Open</Link></div></section>

    <section className="mb-5 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><SectionHeading title="What's for dinner?" action={<Pill tone={meals.length ? "good" : "neutral"}>{meals.length} meals</Pill>} /></div><Group>{meals.length ? meals.map((meal) => <Row key={String(meal.id)}><div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{new Intl.DateTimeFormat("en-GB", { weekday: "short", day: "numeric", month: "short", timeZone: "Europe/London" }).format(new Date(String(meal.date)))}</p><p className="mt-1 text-[15px] font-semibold">{String(meal.meal)}</p></div></div></Row>) : <Row><p className="text-sm text-muted-foreground">No meals are showing for this week yet.</p></Row>}</Group><p className="mt-3 text-[13px] text-muted-foreground">Your meal plan never changes what's actually in your kitchen.</p></section>

    <section className="mb-5 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><SectionHeading title="Shopping" action={<Pill tone={basketStatus === "APPROVED" ? "good" : "attention"}>{statusLabel(basketStatus)}</Pill>} /></div><Group><Row><div className="flex items-center justify-between gap-3"><div><p className="text-[15px] font-semibold">{basket?.status === "READY" ? basket.basket.retailer : "This week's shop"}</p><p className="mt-1 text-[13px] text-muted-foreground">{basket?.status === "READY" ? `${basket.basket.lines.length} things to buy` : basket?.detail ?? "Shopping details aren't available yet."}</p></div><Link to="/shop" className="rounded-md border px-3 py-2 text-[13px] font-medium">View shop</Link></div></Row></Group></section>

    <section className="mb-5 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><SectionHeading title="Delivery" action={<Pill tone={deliveryStatus === "APPROVED" ? "good" : "attention"}>{statusLabel(deliveryStatus)}</Pill>} /></div><Group><Row><div className="flex items-center justify-between gap-3"><div><p className="text-[15px] font-semibold">{delivery?.status === "READY" ? delivery.basket.retailer : "Your delivery"}</p><p className="mt-1 text-[13px] text-muted-foreground">{delivery?.status === "READY" ? "Check what arrived and confirm any changes." : delivery?.detail ?? "Delivery details aren't available yet."}</p></div><Link to="/delivery" className="rounded-md border px-3 py-2 text-[13px] font-medium">Check delivery</Link></div></Row></Group></section>

    <section className="mb-7 rounded-2xl border border-border bg-card p-4 sm:p-5"><SectionHeading title="Something changed?" /><Group><Row><div className="flex flex-wrap gap-2"><Link to="/food" className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground">Update food</Link><Link to="/sweep" className="rounded-md border px-3 py-2 text-sm font-medium">Quick check</Link><Link to="/feedback" className="rounded-md border px-3 py-2 text-sm font-medium">Tell FoodOS</Link></div><p className="mt-3 text-[13px] leading-relaxed text-muted-foreground">If something changed, tell FoodOS rather than leaving it to guess.</p></Row></Group></section>
