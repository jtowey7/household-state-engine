...
export const Route = createFileRoute("/delivery")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Confirm an approved delivery, record only exceptions, and review the exact household events before any state change.",
      },
    ],
  }),
...
function DeliveryPage() {
  ...
  useEffect(() => {
    let active = true;
    getCanonicalBasketForShop()
      .then((read) => {
        if (!active) return;
        setBasket(read);
        if (read.status === "READY") {
          setLines(
            read.basket.lines.map((line, index) => ({
              lineId: `${read.basket.basketId}:${index + 1}`,
      ...
      .catch((error) => {
        if (active) setLoadError(error instanceof Error ? error.message : "Unable to load the approved basket.");
      });
    return () => {
      active = false;
    };
  ...
  const setCapturedAtNow = () => {
    // Recorded-at is generated by the front end when the human opens/confirms the delivery.
  };
  ...
  const allArrived = () => {
    setLines((current) => current.map((line) => ({ ...line, state: "ARRIVED", replacementItemKey: "" })));
    setConfirmed(true);
    setResult(null);
  };
  ...
  const prepare = () => {
    ...
    if (!capturedBy.trim() || !orderReference.trim() || !deliveryId.trim() || !dispatchId.trim()) {
      setResult({ ok: false, code: "INVALID_EVIDENCE", detail: "Order reference, delivery ID, dispatch ID and recorder are required before the delivery can be prepared." });
      return;
    }
    if (Number.isNaN(deliveredDate.getTime())) {
      setResult({ ok: false, code: "INVALID_EVIDENCE", detail: "Delivered-at must contain a valid date before the delivery can be prepared." });
      return;
    }
    if (lines.length === 0) {
      setResult({ ok: false, code: "INVALID_EVIDENCE", detail: "The approved basket contains no lines; refusing to create an empty delivery." });
      return;
    }
    const unresolvedSubstitution = lines.find((line) => line.state === "SUBSTITUTED" && !line.replacementItemKey.trim());
    if (unresolvedSubstitution) {
      setResult({ ok: false, code: "INVALID_EVIDENCE", detail: `A replacement canonical item is required for ${unresolvedSubstitution.productName}.` });
      return;
    }
    ...
    <div className="ctl-page">
      ...
      <Shell>
        ...
        <PageTitle
          ...
          title="Did the delivery arrive?"
          lede="Food OS starts from the approved basket and assumes everything arrived. You only need to tell it about exceptions."
        />
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                Confirming a delivery prepares canonical HOUSEHOLD EVENTS only. Nothing is written to Production until the exact proposed events receive explicit human approval.
              </p>
            </div>
          </div>
        </div>
        ...
        {basketReady ? (
          <>
            <section className="mb-7">
              <SectionHeading title="Approved shop" action={<Badge variant="outline">{basket.basket.retailer}</Badge>} />
              <Group>
                <Row>
                  <p className="text-sm font-semibold">{basket.basket.basketId}</p>
                  <p className="mt-1 text-xs text-muted-foreground">Approved basket · version {basket.approval.basketVersion} · {basket.basket.lines.length} lines</p>
                </Row>
            ...
            <section className="mb-7">
              <SectionHeading title="What actually arrived" action={<Badge>{exceptionCount === 0 ? "No exceptions" : `${exceptionCount} exception${exceptionCount === 1 ? "" : "s"}`}</Badge>} />
              <div className="mb-4 rounded-2xl border border-border bg-card p-4">
                <p className="text-sm font-semibold">Normal case</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">If the delivery was complete, use one click. Food OS will treat every approved basket line as delivered.</p>
                <Button type="button" className="mt-3" onClick={allArrived}>✓ Everything arrived</Button>
                {confirmed ? <p className="mt-2 text-xs font-medium">Delivery defaults set to arrived. Change only the exceptions below.</p> : null}
              </div>
                  ...
                  <Row key={line.lineId}>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">{line.orderedQuantity} {line.unit} · {line.itemKey}</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {(["ARRIVED", "MISSING", "SUBSTITUTED"] as const).map((state) => (
                          <Button key={state} type="button" variant={line.state === state ? "default" : "outline"} onClick={() => updateLine(index, { state, replacementItemKey: state === "SUBSTITUTED" ? line.replacementItemKey : "" })}>
                            {state === "ARRIVED" ? "Arrived" : state === "MISSING" ? "Missing" : "Substituted"}
                          </Button>
                        ))}
                      </div>
                    </div>
                    {line.state === "SUBSTITUTED" ? (
                      <Input className="mt-3" aria-label={`Replacement for ${line.productName}`} value={line.replacementItemKey} onChange={(e) => updateLine(index, { replacementItemKey: e.target.value })} placeholder="Canonical household item that arrived" />
                    ) : null}
                  </Row>
                ))}
              </Group>
          ...
          <section className="mb-7">
            ...
            {result.ok ? (
              <>
                ...
                <Evidence label="Why nothing changed">Preparing this proposal is non-mutating. Explicit human approval bound to the exact Event ID and payload hash is still required before the protected append boundary can write household state.</Evidence>
              </>
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Supermarket checkout remains manual. Delivery confirmation is a household-state input; substitutions are reconciled as the actual item received, and the same governed intake model can also represent food brought home outside a supermarket delivery.
        </p>
      </Shell>
      <AppFooter />
    </div>
