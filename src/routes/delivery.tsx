...
function DeliveryPage() {
  ...
  return <div className="ctl-page"><AppHeader eyebrow="Household" /><Shell>
    ...
    {releaseResult ? <section className="mb-7"><SectionHeading title="Household state update" action={releaseResult.ok && releaseResult.written ? <Badge><CheckCircle2 className="mr-1 size-3" /> Written</Badge> : <Badge variant="destructive">Not written</Badge>} />{releaseResult.ok ? <><p className="text-sm">{releaseResult.written ? `${releaseResult.appended} household event${releaseResult.appended === 1 ? "" : "s"} appended successfully.` : "No household event was written."}</p>{releaseResult.receipts.map((receipt) => <div key={receipt.receiptId} className="mt-2 rounded-xl border border-border bg-card p-3"><p className="text-xs font-semibold">{receipt.eventId}: {receipt.outcome}</p>{receipt.rejection ? <p className="mt-1 text-[13px] leading-relaxed text-destructive">{receipt.rejection.code}: {receipt.rejection.detail}</p> : null}</div>)}{!releaseResult.written ? <Evidence label="Nothing changed">FoodOS did not write household state. The rejection above is the reason; no stock was added by this attempt.</Evidence> : <Evidence label="Next">The canonical HOUSEHOLD EVENTS ledger is now the source for the household inventory readout. Return to the food view to see the resulting stock state.</Evidence>}</> : <Evidence label="Release refused">{releaseResult.detail}</Evidence>}</section> : null}
