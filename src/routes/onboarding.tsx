...
const emptyDraft: Draft = { people: "", equipment: [], constraints: [], constraintNote: "", interests: [], shoppingCadence: "", seasonal: [], recurring: [], note: "" };
      ...
      <Shell>
        ...
        <section className="mb-7 rounded-2xl border border-border bg-card p-4 sm:p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <SectionHeading title="Anything else FoodOS should know?" />
              <p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">
                Optional. Short notes, routines or things that do not fit above.
              </p>
            </div>
            <Pill tone="neutral">Optional</Pill>
          </div>
          <textarea
            aria-label="Anything else FoodOS should know?"
            value={draft.note}
            onChange={(event) => setDraft((current) => ({ ...current, note: event.target.value }))}
            placeholder="e.g. we often have extras on Fridays; prefer smaller peppers"
            rows={3}
